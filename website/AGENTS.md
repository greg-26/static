...
- Use progressive disclosure for execution docs: read `VISION_EXECUTION.md` for routing, then `docs/working-style.md`, then `sprints/INDEX.md`, then only the current sprint file. Read archived logs only for auditing history or debugging a regression.
...
Do not duplicate a stale feedback checklist here. Current acceptance criteria live in `VISION.md`; active execution follow-ups live in `sprints/INDEX.md` and the current sprint file.
