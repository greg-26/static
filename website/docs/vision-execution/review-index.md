# Review and report index

Read the linked report only when the current task explicitly touches that feedback area.

## CEO feedback

- [`reports/ceo/2026-07-13-alex-ceo-feedback.md`](../../reports/ceo/2026-07-13-alex-ceo-feedback.md) — chip dropdowns/state color, From-your-lists row-title control, availability row titles, movie-detail hierarchy.
- [`reports/ceo/2026-07-13-manage-list-share-settings-density-ceo-feedback.md`](../../reports/ceo/2026-07-13-manage-list-share-settings-density-ceo-feedback.md) — manage-list Share behavior, Settings index density, CEO feedback routing.
- [`reports/ceo/2026-07-13-settings-lists-open-copy-share-ceo-feedback.md`](../../reports/ceo/2026-07-13-settings-lists-open-copy-share-ceo-feedback.md) — Settings → Lists row navigation and clipboard-first copy feedback.

## Planning and QA

- [`reports/principal-engineer/2026-07-13-principal-engineer-plan.md`](../../reports/principal-engineer/2026-07-13-principal-engineer-plan.md) — implementation sequencing and risk plan.
- [`reports/pmt/2026-07-13-pm-tech-vision-cx-report.md`](../../reports/pmt/2026-07-13-pm-tech-vision-cx-report.md) — PM-tech vision/CX findings.
- [`reports/qa-deep-dive/2026-07-13-principal-sde-qa-report.md`](../../reports/qa-deep-dive/2026-07-13-principal-sde-qa-report.md) — QA deep-dive findings.

## Rule

Do not paste report contents into `VISION_EXECUTION.md`. Link reports here and summarize only active consequences in `current-status.md`.
