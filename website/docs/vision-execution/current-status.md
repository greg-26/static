...
- Latest remaining priority: Sprint 9 trust/accessibility items above. Earlier CEO feedback on chip color semantics, row-header selector, availability-title cleanup, duplicate detail availability, cross-profile suitability, and Settings list copy/open behavior is implemented locally; keep only manual phone/touch verification as needed.
