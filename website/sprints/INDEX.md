...
- Current implementation phase: Sprints 018–022 are complete and archived; Sprint 023 is the next ready implementation slice.
- Next executable sprint: Sprint 023 — movie-detail top actions and poster affordance.
...
| --- | --- | --- | --- |
| 023 | Move list actions into the first movie-detail decision area and add a mobile-safe way to inspect the poster larger. | `ready` | Current movie-detail modal/list persistence remains stable |
| 024 | Polish movie-detail media surfaces: sharp/blended backdrop, tighter internal collection/season cards, and one-row circular cast. | `ready` | Sprint 023 complete enough not to fight the same header/detail layout |
...
- Issue #19 is implemented, verified on mobile-width movie details, and closed by the implementation workflow with evidence.
- Issue #20 is implemented with a keyboard/touch-accessible larger-poster affordance and closed with evidence.
